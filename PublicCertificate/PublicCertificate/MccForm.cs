...
{
    public partial class MccForm : Mcc.Series.Ui.FormBase
    {
        enum Status
        {
            OLD,
        ...
        private void BtnDbCert_Click(object sender, EventArgs e)
        {
            ...
            BringCert bringCert = new BringCert();
            dt.Columns.Add("filename");
            dt.Columns.Add("path");
            dt.Columns.Add("expire_date");
            dt.Columns.Add("expire_gb");
            dt.Columns.Add("date");
            ...
            if (userDi.Exists)
            {
                ...
                for (int i = 0; i < directories.Length; i++)
                {
                    ...
                    if (file.Name.Contains("cn="))
                    {
                        ...
                        string szSignCertFile = directories[i] + @"\signCert.der";

                        X509Certificate2 cert = new X509Certificate2(szSignCertFile);
                        byte[] byteCert = File.ReadAllBytes(szSignCertFile);
                        X509CertificateStructure xCert = X509CertificateStructure.GetInstance(Asn1Object.FromByteArray(byteCert));
                        ...
                        if (DateTime.Compare(now, expireDate) > 0) { gb = "만료"; }

                        dt.Rows.Add(fileNM[1].Substring(0, fileNM[1].Length - 3), file.FullName, ExpDate, gb, file.CreationTime);
                    }
                }
            }

            bringCert.SetDataTable = dt;
            bringCert.Text = "공동인증서 선택";
            bringCert.Width = 700;
            bringCert.Height = 350;
            bringCert.searchType = 0;
            bringCert.ColCount = 5;
            bringCert.Load += delegate
            {
                bringCert.grdSavedCert.DisplayLayout.Override.RowSelectors = Infragistics.Win.DefaultableBoolean.False;
            };
            bringCert.ColNames = "이름,경로,만료일자,구분,수정일자";
            bringCert.ColSizes = "150,250,80,50,130";

            bringCert.UseColumns = "filename,path,expire_date,expire_gb,date";

            if (bringCert.ShowDialog() == DialogResult.OK)
            {
                if (bringCert.ReturnName != txtSaveLocation.Text)
                {
                    txtLoginPwd.Text = "";
                    chkAutoLogin.Checked = false;
                    txtSaveLocation.Text = bringCert.ReturnName;
                }
            }
        ...
        private void BtnDown_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("인증서를 받으시겠습니까", "내려받기", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        if (dr["gb"].ToString().Equals("1"))
                        {
                            ...
                            if (dir.Exists)
                            {
                                ...
                                backup_path = "C:\\Users\\" + Environment.UserName.ToString() + "\\AppData\\LocalLow\\NPKI\\KICA\\USER\\" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + "\\" + dr["file_nm"].ToString();
                    ...
                    txtCertificateLocation.Text = backup_path;
        ...
        private void SaveData()
        {
            ...
            try
            {
                ...
                DirectoryInfo di = new DirectoryInfo(txtSaveLocation.Text);
